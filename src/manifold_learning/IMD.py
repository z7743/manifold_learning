...
class IMD_nD:
    ...
    def __init__(self, input_dim, embed_dim, n_components, subtract_corr=True, learning_rate=0.001,device="cuda",optimizer="Adam",random_state=None):
        self.device = device
    ...
    def fit(self, X, sample_len, library_len, exclusion_rad, nbrs_num, tp, epochs=10, num_batches=32):
    ...
    def loss_fn(self, subset_idx, sample_idx, sample_td, sample_pred, subset_td, subset_pred, nbrs_num, exclusion_rad):
        ...
        if self.subtract_corr:
            corr = torch.abs(self.get_autoreg_matrix_approx(sample_td, sample_pred))
            if dim > 1:
