...
from .data_samplers import RandomTpRangeSubsetDataset_2p

class IMD_nD_2p_smap:

    def __init__(self, input_dim_X, input_dim_Y, embed_dim, n_components, subtract_corr=True,device="cuda",random_state=None):
        """
        ...
        self.model_X = LinearProjectionNDim(input_dim_X, embed_dim, n_components, device,random_state)
        self.model_Y = LinearProjectionNDim(input_dim_Y, embed_dim, n_components, device,random_state)
    ...
    def fit(self, X, Y,
        ...
        if tp_policy == "fixed":
            dataset = RandomTpRangeSubsetDataset_2p(X,Y, sample_len, library_len, num_batches, torch.linspace(tp, tp+1 - 1e-5,num_batches,device=self.device).to(torch.int),device=self.device)
        elif tp_policy == "range":
            dataset = RandomTpRangeSubsetDataset_2p(X,Y, sample_len, library_len, num_batches, torch.linspace(0, tp+1 - 1e-5,num_batches,device=self.device).to(torch.int), device=self.device)
        else:
        ...
        if (self.learning_rate != learning_rate) or (self.optimizer_name != optimizer):
            ...
            self.optimizer = getattr(optim, optimizer)(
                    list(self.model_X.parameters()) + list(self.model_Y.parameters()),
                    lr=learning_rate
            )
        ...
        for epoch in range(epochs):
            ...
            for subset_idx, sample_idx, subset_X, subset_y, sample_X, sample_y, subset_Y, subset_x, sample_Y, sample_x in dataloader:
                subset_X_z = self.model_X(subset_X)
                subset_y_z = self.model_Y(subset_y)
                sample_X_z = self.model_X(sample_X)
                sample_y_z = self.model_Y(sample_y)
                ...
                loss = self.loss_fn(subset_idx, sample_idx, sample_X_z, sample_y_z, subset_X_z, subset_y_z, 
                                    sample_Y_z, sample_x_z, subset_Y_z, subset_x_z,
    ...
    def loss_fn(self, subset_idx, sample_idx, sample_X, sample_y, subset_X, subset_y, sample_Y, sample_x, subset_Y, subset_x, theta, exclusion_rad, loss_mask_size):
        ...
        ccm_XY = (self._get_ccm_matrix_approx(subset_idx, sample_idx, sample_X, sample_y, subset_X, subset_y, theta, exclusion_rad))
        #ccm_XX = (self._get_ccm_matrix_approx(subset_idx, sample_idx, sample_X, sample_x, subset_X, subset_x, theta, exclusion_rad))
        ccm_YX = (self._get_ccm_matrix_approx(subset_idx, sample_idx, sample_Y, sample_x, subset_Y, subset_x, theta, exclusion_rad))
        #ccm_YY = (self._get_ccm_matrix_approx(subset_idx, sample_idx, sample_Y, sample_y, subset_Y, subset_y, theta, exclusion_rad))
        ccm = (ccm_XY + ccm_YX)/2
        ...
        if self.subtract_corr:
            ...
            corr_XY = torch.abs(self._get_autoreg_matrix_approx(sample_X,sample_y))
            corr_YX = torch.abs(self._get_autoreg_matrix_approx(sample_Y,sample_x))
            #corr_XX = torch.abs(self._get_autoreg_matrix_approx(sample_X,sample_x))
            #corr_YY = torch.abs(self._get_autoreg_matrix_approx(sample_Y,sample_y))
            corr = (corr_XY + corr_YX)/2
    ...
    def predict_X(self, X):
        ...
        with torch.no_grad():
            ...
            outputs = torch.permute(self.model_X(inputs),dims=(0,2,1)) #Easier to interpret
    ...
    def predict_Y(self, Y):
        ...
        with torch.no_grad():
            ...
            outputs = torch.permute(self.model_Y(inputs),dims=(0,2,1)) #Easier to interpret
        return outputs.cpu().numpy()
